...
function loadEvent(file: string) {
  try {
    ...
    events.set(
      `${clazz.event.toUpperCase()}-${clazz.type.toUpperCase()}`,
      clazz
    );
  } catch (e) {
    console.error(
...
export function handleEvent(socket: AuthSocket, event: string, args: any[]) {
  const socketEvent: SocketEvent | undefined = events.get(
    `${event.toUpperCase()}-${socket.type}`
  );
  ...
  if (socketEvent.parameters.length != 0) {
    ...
    for (const arg of args) {
      ...
      // Edge Cases
      if (required == "buffer" && (arg == null || arg instanceof Buffer))
        required = "object";
      if (
        required == "uint8array" &&
        (arg == null || arg instanceof Uint8Array)
      )
        required = "object";
      if (["ack", "acknowledgement", "func", "function"].includes(required))
        required = "function";
      ...
      if ((typeof arg).toLowerCase() != required) {
        return socket.emit(
          ...
          `Argument #${
            index + 1
          } (${typeof arg}) does not match the required type: ${required}`
        );
      }
      index++;
    }
