...
export default class CopyFileEvent extends SocketEvent {
	override readonly event = 'COPY_FILE';
	override readonly type = 'PANEL';
	override readonly parameters = ['string', 'string', 'string', 'boolean', 'ack'];

	override async onEvent(socket: AuthSocket, server: string, from: string, to: string, root: boolean, acknowledgement) {
		...
		if (!hasPermission(minecraft, ServerPermission.WRITE_FILES)) return socket.emit('error', 'You are not permitted to do this!');
		...
		await copyFile(serverSocket, from, to, root);
		acknowledgement();
	}
}
