...
export default class KickPlayerEvent extends SocketEvent {
	override readonly event = 'KICK_PLAYER';
	override readonly type = 'PANEL';
	override readonly parameters = ['string', 'string', 'string'];

	override async onEvent(socket: AuthSocket, server: string, uuid: string, reason: string) {
		...
		kickPlayer(serverSocket, uuid, reason);
	}
}
