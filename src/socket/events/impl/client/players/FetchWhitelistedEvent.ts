...
import { getWhitelistedPlayers } from '../../../../utils';

export default class FetchWhitelistedEvent extends SocketEvent {
	override readonly event = 'FETCH_WHITELISTED_PLAYERS';
	...
	override async onEvent(socket: AuthSocket, server: string, acknowledgement) {
		...
		const players = await getWhitelistedPlayers(serverSocket);
