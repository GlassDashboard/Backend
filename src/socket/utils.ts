...
export async function getAllFiles(
  ...
  return new Promise((resolve, _) => {
    server
      .timeout(5000)
      .emit("ALL_FILES", JSON.stringify({ path, root }), (err, file) => {
...
export async function getFileData(
  ...
  return new Promise((resolve, _) => {
    server
      .timeout(5000)
      .emit("FETCH_FILE", JSON.stringify({ path, root }), (err, file) => {
        if (err) return resolve(null);
...
export function createFile(
  ...
  return new Promise((resolve, _) => {
    server
      .timeout(5000)
      .emit("CREATE_FILE", JSON.stringify({ path, root }), () => {
        resolve(undefined);
      });
  });
}
...
export function unarchiveFile(
  ...
  return new Promise((resolve, _) => {
    server
      .timeout(5000)
      .emit("UNARCHIVE_FILE", JSON.stringify({ path, root }), () => {
        resolve(undefined);
      });
  });
}
...
export function deleteFile(
  ...
  return new Promise((resolve, _) => {
    server
      .timeout(5000)
      .emit("DELETE_FILE", JSON.stringify({ path, root }), () => {
        resolve(undefined);
      });
  });
}
