...
export default async function handleAuthentication(
  ...
  if (!auth || !auth.type)
    return next(new Error("No authentication or type parameters provided."));
  if (!["panel", "plugin"].includes(auth.type.toLowerCase()))
    return next(new Error("Invalid socket type provided."));
  ...
  if (authSocket.type == "PANEL") {
    ...
    if (!auth.discord)
      return next(new Error("No discord authentication was passed."));

    const discord: Discord | null = await getDiscord(auth.discord);
    if (!discord)
      return next(new Error("Invalid or unverified discord provided."));
  ...
  if (authSocket.type == "PLUGIN") {
    ...
    if (!auth.minecraft || !types.includes(auth.minecraft.toUpperCase()))
      return next(new Error("Invalid server type recieved."));

    // Ensure server authentication token provided is real
    if (!auth.token)
      return next(
        new Error(
          "Invalid server token was provided. Please set one in the config.yml"
        )
      );
    ...
    if (!minecraft)
      return next(
        new Error(
          "Invalid server token provided. Make sure you provided the correct token in config.yml"
        )
      );
    ...
    if (authSocket.minecraft.serverType != auth.type.toUpperCase())
      startup.type = auth.minecraft.toUpperCase();
  ...
  next(
    new Error("An unexpected error has occurred. Please contact a developer.")
  );
}
