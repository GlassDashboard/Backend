...
export default async function handleAuthentication(socket: Socket, next: (err?: ExtendedError) => void) {
	...
	if (authSocket.type == 'PLUGIN') {
		...
		// Get and validate the server token & version
		...
		// Close any existing connections with the same server
		onlineServers.get(minecraft._id)?.disconnect(true);
		...
		if (authSocket.minecraft.serverType != auth.type.toUpperCase()) startup.serverType = auth.minecraft.toUpperCase();
