...
router.get("/:server", loggedIn, async (req: Request, res) => {
  ...
  res.json({
    ...
    server: {
      ...
      token: hasPermission(server, ServerPermission.MANAGE_SERVER)
        ? server.token
        : undefined,
      ftp: hasPermission(server, ServerPermission.MANAGE_SERVER)
        ? server.ftp
        : undefined,
    },
  });
...
router.get("/", loggedIn, async (req: Request, res) => {
  ...
  res.json({
    ...
    servers: accessible.map((s: ClientMinecraftServer) => {
      return {
        ...s,
        token: hasPermission(s, ServerPermission.MANAGE_SERVER)
          ? s.token
          : undefined,
...
router.post("/:server", loggedIn, async (req: Request, res) => {
  ...
  const existing = await ServerModel.findById(
    req.params.server.toLowerCase().replace(" ", "")
  );
  if (existing)
    return res
      .status(403)
  ...
  const minehut = await fetch(
    `https://api.minehut.com/server/${req.params.server}?byName=true`
  ).then((res) => res.json());
  ...
  if (minehut.server.proxy)
    return res
      .status(403)
      .json({ error: true, message: "We do not currently support proxies!" });
    ...
    if (!updated)
      return res
        ...
        .json({
          error: true,
          message:
            "Failed to update on database. Please report this to a developer!",
          code: 2,
        });
