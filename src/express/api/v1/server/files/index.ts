...
router.get('/download/:path*', requiresPermission(ServerPermission.READ_FILES), async (req, res) => {
	...
	if (!path) return res.status(400).json({ error: true, message: 'An internal error has occurred', code: 2 });
	...
	if (!file) return res.status(404).json({ error: true, message: 'This file does not exist!' });
...
router.post(['/upload/:path*', '/upload'], requiresPermission(ServerPermission.WRITE_FILES), async (req, res) => {
    ...
    for (const file of files) {
        ...
        while (offset < file.data.length) {
            stream._write(file.data.slice(offset, offset + 1024 * 1024) as Buffer, 'binary', () => {});
            offset += 1024 * 1024;
        }
	...
	return res.json({ error: false, message: `Successfully uploaded ${files.length} files`, files });
});

router.get('/:path*', requiresPermission(ServerPermission.READ_FILES), async (req, res) => {
	const auth = req as AuthenticatedRequest;
	const accessible: ClientMinecraftServer[] = await User.getAssociatedServers(auth.discord.id);
