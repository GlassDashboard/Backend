...
router.get('/confirm/:invite', loggedIn, ratelimit(3, '10s'), ratelimit(50, '1m', true), async (req: Request, res) => {
	...
	const data: DocumentType<Invite> | null = await InviteModel.findById(invite);
    ...
    if (data.adminOnly) {
        ...
        if (user == null || !user.admin) return res.status(403).json({ error: true, message: 'You are not permitted to accept this invite!' })
    }
	...
	if (user.invite != undefined) return res.status(400).json({ error: true, message: 'You have already used an invite.' });
