...
server.on(
  "login",
  async ({ connection, username, password }, resolve, reject: any) => {
    ...
    if (connectionDetails.length != 2) {
      await connection.reply(
        400,
        "[Glass] Invalid connection details, double check your username and password"
      );
    ...
    const server: AuthSocket | undefined = onlineServers.get(
    ...
    const ftpDetails: FTPDetails | undefined = server.minecraft!.ftp.find(
      (ftp) => {
        return (
          ftp.identifier == connectionInfo.identifier &&
