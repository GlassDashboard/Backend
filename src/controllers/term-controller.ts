...
export class TermController {
	...
	async getConsole(
		...
		@Server({ permissions: [ServerPermission.VIEW_CONSOLE] }) server: UserServer
	) {
	...
	async sendCommand(
		@CurrentUser() user: User,
		@Server({ permissions: [ServerPermission.USE_CONSOLE] }) server: UserServer,
		@Body() data: CommandData
	) {
		const socket = server.getAsUser(user).getSocket();
		if (!socket) throw new HttpError(500, 'Server socket not found.');
